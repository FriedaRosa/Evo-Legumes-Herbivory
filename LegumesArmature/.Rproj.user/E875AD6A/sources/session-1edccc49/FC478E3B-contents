...
rm(tree2, tree3, tree4, traitData, new_tip_names, mimosoidPhylo, armature, matches, sp_drop)
...
plot (a2na, col = myColours[1], xlab = "Time of transitions (mya)",  
      ...
      ylim = c(0, 8), xlim = c(100,0)) # alter if x values change!
...
plot(density_a2na, lwd = 2, col = myColours[1], 
     xlim = c(100,0), xlab = "Time of transitions (mya)", bty = "l",
     cex.lab = 1.4, cex.axis = 1.4, main = NULL, sub = NULL, title = NULL)
lines(density_na2a, lwd = 2, col = myColours[2], xlim = c(100,0))
