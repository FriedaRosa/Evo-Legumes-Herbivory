...
The following code is a mixture of code written by **Liam Revell within
the Phytools package**, especially from the source code of the ctt()
function (version Oct 2023),

and from the code for Figure 4 in **Stephens et al., (2023):**
[Paper](https://nph.onlinelibrary.wiley.com/doi/10.1111/nph.18993);
[GitHub
Repo](https://github.com/rubysaltbush/pollination-macroevolution)
...
# 1. My Data (read back in from RDS)

## A) Legumes

```{r}
#| label: Legumes data & simmap


legumes_tree <- ladderize(readRDS("../input/Legumes/Processed/Legumes_MCC_drop.rds"))



legumes_simmap_100 <- readRDS("../output/RDS/Legumes_simmap_100.rds")
ladderize(legumes_simmap_100)

 obj <- ltt(legumes_simmap_100[[1]], plot=F)


 Armature <- as.factor(rownames(unique(legumes_simmap_100[[1]]$Q)))
 
 layout(matrix(c(1,2),2,1),heights=c(0.4,0.6))
cols<-setNames(c("gold", "grey", "black"),c(levels(Armature),
    "total"))
plot(legumes_simmap_100[[1]],cols,ftype="off",lwd=2,
    mar=c(0,4.1,1.1,1.1))
grid()
...
plot(obj,colors=cols,bty="n",las=1,lwd=4,show.tree=FALSE,
    legend=FALSE,ylim=c(1,Ntip(legumes_tree)),
...
```{r}
#| labalphl: Mammals data & simmap


mammals_tree <- readRDS("../input/Mammals/Processed/Mammals_MCC_drop.rds")
mammals_simmap_100 <- readRDS("../output/RDS/Mammals_simmap_100.rds")
mammals_simmap_100_4 <- readRDS("../output/RDS/Mammals_4traits_simmap_100.rds")
```

Now we have 2 Simmap RDS objects: each with 100 simulations on the MCC
tree (mtrees)
...
Based on: Ref\[0\]: [Pollination-Macroevolution
Study](https://github.com/rubysaltbush/pollination-macroevolution/),
which was based on Ref\[1\]: Liam Revell's [Blog
Post](http://blog.phytools.org/2015/08/getting-timing-of-trait-changes-from.html).

Transition_times function comes from script Ref\[2\]:
[transition_times.R](https://github.com/rubysaltbush/pollination-macroevolution/blob/main/scripts/functions/transition_times.R)

Code below function comes from script Ref\[3\]:
...
Now we set our colours for plotting and add some transparancy for the
simulation data:

```{r}
#| label: Colors for Armature
...
First we will get an overview of the transitions by plotting a histogram
and a density graph where x is the the average time of transitions and y
is the density of it. The figure is saved in the output/Figs/ folder.
...
Next we will plot the cumulative transition rates. x = time of the
transition y = number of transitions (naturally grows with time) The
figure is saved in the output/Figs/ folder.
...
write.csv(avg_trans_df, "../output/CSV/Average_transition_times_4traits_mammals.csv")
...
par(mar=c(20,4,4,4));plot(mtrees[[1]]); axisPhylo()
```
...
transition_states <- unique(trans_avg_length$transition)
library(viridis)
viridis(n=10)
avg_trans_df


myColours = c("darkgrey", "darkgrey", "lightgrey", "#FFBB00", "lightgrey")

myColours = c(viridis(n=10))
myColoursAlpha <- add.alpha(myColours, alpha=0.2)
my_cols <- setNames(myColoursAlpha, transition_states)




ggplot(data = avg_trans_df, 
       aes(y = as.factor(transition), x = avg_time, fill = as.factor(transition) ))+ 
  geom_boxplot()
...
for (i in seq_along(unique(trans_avg_length$transition))){ # i in 10
  type <- unique(trans_avg_length$transition)[i]
  trans_i <- avg_trans_times %>%
    dplyr::filter(transition == type) 
  
  histo <- hist(trans_i$avg_time, breaks = ax, plot = FALSE)
  p_histo <- plot (histo, col = myColours[1], xlab = "Time of transitions (mya)",  
      main = "", ylab = "number of transitions", 
      ylim = c(0, 30), xlim = c(220,0))

  
  
  
  density_i <- megaherbivore_transitions %>%
  dplyr::filter(transition == type) %>%
  dplyr::mutate(other_to_mega = time) %>%
  dplyr::select(transition, time, simulation = i)
  
  # calculate density curve
  density_p <- density(density_i$time)
  # plot the density
  p_density <- plot(density_p, lwd = 2, col = myColours, 
     xlim = c(220,0), xlab = "Time of transitions (mya)", bty = "l", ylim=c(0,0.1),
     cex.lab = 1.4, cex.axis = 1.4, main = NULL, sub = NULL, title = NULL);
  title(main=paste0(type))
  
  hist_list[[i]] <- list(p_histo, p_density, density_p)
...
Next we will plot the cummulative transition rates. x = time of the
transition y = number of transitions (naturally grows with time) The
figure is saved in the output/Figs/ folder.
